...
# =========================
MODEL = "gpt-4o-mini"
WINDOW_NAME = "Smart Cane (S=Start Live, V=Voice Question, H=Help, Q=Quit)"
...
# =========================
# AI description (GitHub Models)
# =========================
def describe_frame(client, frame, reason, distance_cm, custom_question=None):
...
def main():
    ...
    def listen_and_ask(frame_to_analyze, dist):
        state["is_listening"] = True
        state["status"] = "Listening (5 sec)..."

        try:
            print("\n[SYSTEM] Mic is ON! Recording for 5 seconds...")
            fs = 44100
            seconds = 5
            ...
            question_text = recognizer.recognize_google(audio_data, language="en-US")
    ...
    print(
        "Smart Cane Interactive started. Press S for Live Mode, V to Speak, Q to quit."
    )
        ...
        while True:
            ...
            if key == ord("s") or key == ord("S"):
                ...
                if not state["is_listening"]:
                    threading.Thread(
                        target=listen_and_ask,
                        args=(frame.copy(), distance_cm),
                        daemon=True,
                    ).start()
